...
function showAllThemesCourse() {
	$('#div-modules').addClass('row');
	$.post(
		'../php/information-course.php',
		{
			option: 'all themes course',
			course_key: id_course,
		},
		function (response) {
			var json = JSON.parse(response);
			var tam = json.all_themes_course.length;
			for (var i = 0; i < tam; i++) {
				$('#div-themes').append(
					"<div class='col-md-12 col-sm-12'>" +
						"<div class='card mt-3 card-modules'>" +
						"<div class='card-body'>" +
						json.all_themes_course[i].name +
						'</div>' +
						'</div>' +
						'</div>'
				);
			...
			success: function (response) {
				...
				if (text == json.signed_up.title) {
					Swal.fire({
						...
						text:
							'Ya estas inscrito, no puedes volver a inscribirte, accede a tu lista de cursos para que lo puedas ver',
						icon: 'warning',
				...
				Swal.fire({
					...
					if (result.value) {
						Swal.fire({
							...
							text: 'Ahora puedes realizar el curso',
							icon: 'success',
							confirmButtonColor: '#092432',
	...
	$.ajax({
		...
		success: function (response) {
			...
			if (json.url_singned.students == 0 || json.url_singned.students == null) {
				...
				$('#singned-studnets').text(
					json.url_singned.students + ' ' + students + ' ' + singned + ' EN ESTE CURSO'
				);
			}
		},
	});
