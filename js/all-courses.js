        ...
        "<div class='col-md-4 col-sm-12 mt-4'>"+
            "<div class='card'>"+
                "<video class='embed-responsive embed-responsive-16by9' controls>"+
                    "<source src=../"+response.all_courses[i].url_video+" type='video/mp4'>"+
                "</video>"+
                "<div class='card-body'>"+
                    "<h4 class='card-title title-course'>"+response.all_courses[i].title+"</h4>"+
                    "<div class='row'>"+
                        "<div class='col-md-4 col-sm-12'>"+
                            "<h6 class='card-text mt-2 text-danger'>Gratis</h6>"+
                        "</div>"+
                        "<div class='col-md-8 col-sm-12'>"+
                            "<a href='information-course?title="+response.all_courses[i].title+
                            "&desc="+response.all_courses[i].description+"&lessons="+response.all_courses[i].total_lessons+
                            "&hours="+response.all_courses[i].total_hours+"&months="+response.all_courses[i].elapsed_month+
                            "&course="+response.all_courses[i].id_courses+"' class='btn btn-sm btn-success' id='access-course'>Acceder al curso</a>"+
                        "</div>"+
                    "</div>"+
                "</div>"+               
            "</div>"+
        ...
        success: function (response) {
            ...
            if(response != "sin categorias"){
                ...
                for(data = 0; data < size; data++){
                    ...
                    $("#select-category").append("<option value="+json.categories[data].id_category+">"+json.categories[data].name+"</option>");
                }
